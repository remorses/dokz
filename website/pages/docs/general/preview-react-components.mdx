...
export default function Page(props) {
    ...
    return (
        <DokzProvider playgroundScope={() => import('@chakra-ui/core')}>
            <Component {...pageProps} />
        </DokzProvider>
    )
}
...
Here i am using a dynamic import to not slow down the first page load, the playground scope will be downloaded in background and cached via swr
